...
const apis = [
  ...
  const getAnswerFrom = async (question, url2) => {
    ...
    // 获取用户填写的 API Key
    const apiKey = configStore.queryApis[0].token;
    if (!apiKey) {
      return handleError("请在脚本设置中填入 DeepSeek API Key");
    }
    ...
    let userContent = `题目类型: ${typeNames[question.type] || "未知"}\n题目: ${question.title}\n`;

    // 添加选项（如果有）
    if (question.optionsText && question.optionsText.length > 0) {
      userContent += '选项:\n';
      question.optionsText.forEach((opt, idx) => {
        userContent += `${String.fromCharCode(65 + idx)}. ${opt}\n`;
      });
    }

    // 系统提示词：约束 AI 输出格式
    const systemPrompt = `你是一个专业的答题助手。请直接给出答案，不要添加任何解释、分析或多余的标点符号。
规则：
1. 单选题：只回答一个字母，例如：A
2. 多选题：回答字母，用逗号分隔，例如：A,B,C
3. 判断题：只回答"正确"或"错误"
4. 填空/简答：直接给出答案内容`;
    ...
    return new Promise((resolve) => {
      ...
      _GM_xmlhttpRequest({
        ...
        headers: {
          ...
          "Authorization": `Bearer ${apiKey}` // 添加鉴权头
        },
        data: requestData,
        timeout: 60000, // 60秒超时
        onload: (response) => {
          try {
            ...
            if (apiResponse.error) {
               console.error("[DeepSeek] API Error:", apiResponse.error);
               resolve(handleError(`API错误: ${apiResponse.error.message}`));
               return;
            }

            // 解析 DeepSeek 返回的内容
            let aiAnswer = apiResponse.choices?.[0]?.message?.content || "";
            aiAnswer = aiAnswer.trim();

            console.log("[DeepSeek] 原始回答:", aiAnswer);
            ...
            if (question.type === "0") {
              ...
              const mappedAnswers = letters.map(letter => {
                  const idx = letter.charCodeAt(0) - 65;
              ...
              // 判断题、填空题、简答题直接使用原始内容
              // 清理一下可能出现的 "答案：" 前缀
              aiAnswer = aiAnswer.replace(/^(答案|Answer)[:：]\s*/i, "");
              answers = [aiAnswer];
            }
            ...
            if (answers.length > 0) {
              ...
              resolve(handleError("无法解析AI答案"));
            }

          } catch (e) {
            console.error("[DeepSeek] 解析错误:", e);
            resolve(handleError(`解析出错: ${e.message}`));
          }
        },
        onerror: (error) => {
          console.error("[DeepSeek] 请求网络错误:", error);
          resolve(handleError("网络请求失败，请检查网络或代理设置"));
        },
        ontimeout: () => {
          resolve(handleError("DeepSeek 响应超时"));
        }
      });
    });
  };
